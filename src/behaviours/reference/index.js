 ...
 */
export default function referenceBehaviour(referencesNames = []){

    // Arguments validation
    if(isUndefined(referencesNames) || isNull(referencesNames)|| (!isString(referencesNames) && !isArray(referencesNames))){
        throw new Error('the referencesNames should be givent in order to to know which references to pick up from the configuration.');
    }

    // Definition Construction
    const referencesConf = isArray(referencesNames) ? referencesNames : [referencesNames];
